...
**Metric/Perspective Two:** From Ventura's odyssey we also explored the step of Filtration, and idea of self evalutaiton of our system. Our system places value on more similar ingredients. Our generalization model adds ingredients that pair well together (that aren't nessicarily similar), and then our system uses a fitness function that compares all of the ingredients and places a numerical value on the recipe that indecates the total similarity of all the add ins. This allows are system to explore more during the creation process, and then places higher value on recipes with a more cohesive ingredient theme. Even though things that pair well, and similar ingredients seem like similar measures, they function differently and allow us to explore novelty as well as value.  
